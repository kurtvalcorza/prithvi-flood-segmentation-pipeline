...
A local GPU pre-flight execution of the generated notebook completed the whole default path — but it ran with the
Hub files, the converted safetensors and the data objects pre-staged, so neither the 1.28 GB Hub download, the
in-notebook audit and conversion, nor the 104 MB Sen1Floods11 fetch has been exercised end to end by the notebook;
the hosted run must cover all three. The repository stays at **Candidate** until a Colab or fresh-container run of
the exact release revision is recorded above.
